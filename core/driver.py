...
class SeleniumDriver:
    ...
    def dismiss_popups(self) -> None:
        ...
        # CMP / Consent dialogs (multilingual)
        for text in ["Agree", "Acepto", "Accept", "Aceptar", "Accepter", "Accetta", "Akkoord", "Zgadzam się"]:
            self._try_click(By.XPATH, f"//button[text()='{text}']")
            self._try_click(By.XPATH, f"//button//p[text()='{text}']")
            self._try_click(By.XPATH, f"//button[contains(@aria-label, '{text}')]")

        # Specific class-based selector for Funding Choices
        self._try_click(By.XPATH, "//button[contains(@class,'fc-cta-consent')]/p")
        self._try_click(By.XPATH, "//div[contains(@class, 'fc-dialog')]//button[contains(@class, 'fc-primary-button')]")
